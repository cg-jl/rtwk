...
static void random_spheres() {
            ...
            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
                ...
                if (choose_mat < 0.8) {
                    ...
                    world.add(make_shared<transformed_geometry>(
                        ...
                        make_shared<sphere>(center, 0.2, sphere_material, sphere_texture)));
                    ...
                    world.add(
                        make_shared<sphere>(center, 0.2, sphere_material, make_shared<solid_color>(albedo)));
                } else {
                    // glass
                    sphere_material = make_shared<dielectric>(1.5);
                    world.add(
                        make_shared<sphere>(center, 0.2, sphere_material, make_shared<solid_color>(1,1,1)));
                }
            }
        }
    }
    ...
    world.add(make_shared<sphere>(point3(0, 1, 0), 1.0, material1, make_shared<solid_color>(1,1,1)));

    auto material2 = make_shared<lambertian>();
    world.add(make_shared<sphere>(point3(-4, 1, 0), 1.0, material2, make_shared<solid_color>(.4,.2,.1)));

    auto material3 = make_shared<metal>(0.0);
    world.add(make_shared<sphere>(point3(4, 1, 0), 1.0, material3, make_shared<solid_color>(color(0.7,.6,.5))));
...
static void earth() {
    ...
    auto globe = make_shared<sphere>(point3(0, 0, 0), 2, earth_surface, earth_texture);
...
static void simple_light() {
    ...
    world.add(make_shared<sphere>(point3(0, 7, 0), 2, difflight, difflight_color));
...
static void cornell_box() {
    ...
    world.add(make_shared<quad>(point3(555, 0, 0), vec3(0, 555, 0),
                                vec3(0, 0, 555), make_shared<lambertian>(), green));
    world.add(make_shared<quad>(point3(0, 0, 0), vec3(0, 555, 0),
                                vec3(0, 0, 555),  make_shared<lambertian>(), red));
    ...
    world.add(make_shared<quad>(point3(0, 0, 0), vec3(555, 0, 0),
                                vec3(0, 0, 555), make_shared<lambertian>(), white));
    world.add(make_shared<quad>(point3(555, 555, 555), vec3(-555, 0, 0),
                                vec3(0, 0, -555), make_shared<lambertian>(), white));
    world.add(make_shared<quad>(point3(0, 0, 555), vec3(555, 0, 0),
                                vec3(0, 555, 0),  make_shared<lambertian>(),white));

    shared_ptr<hittable> box1 =
        box(point3(0, 0, 0), point3(165, 330, 165), make_shared<lambertian>(), white);
    ...
    shared_ptr<hittable> box2 =
        box(point3(0, 0, 0), point3(165, 165, 165), make_shared<lambertian>(), white);
...
static void cornell_smoke() {
    ...
    world.add(make_shared<quad>(point3(113, 554, 127), vec3(330, 0, 0),
                                vec3(0, 0, 305), make_shared<diffuse_light>(), light));
...
static void final_scene(int image_width, int samples_per_pixel, int max_depth) {
    ...
    world.add(make_shared<sphere>(point3(260, 150, 45), 50,
                                  make_shared<dielectric>(1.5), make_shared<solid_color>(1,1,1)));
    world.add(
        make_shared<sphere>(point3(0, 150, 145), 50,
                            make_shared<metal>(1.0), make_shared<solid_color>(color(0.8, 0.8, 0.9))));
    ...
    world.add(make_shared<sphere>(point3(400, 200, 400), 100, make_shared<lambertian>(), emat));
    ...
    int ns = 1000;
    for (int j = 0; j < ns; j++) {
        boxes2.add(make_shared<sphere>(point3::random(0, 165), 10, make_shared<lambertian>(), white));
    }
